	...
	if(checkphoneNoFlag){
		$.ajax({
			...
			success : function(generation){
		   		if(checkphoneNoFlag){
	    				var num=60;
	    				var Span=document.getElementsByClassName('order_box')[0].getElementsByTagName('span')[1];
   ...
   if(checkphoneNoFlag){
	 ...
	 $.ajax({
	    ...
	    success : function(data){
	    	if(data=="1"){   //用户已存在(校验验证码并判断是否绑定微信)
	    		$.ajax({
					   url : getUrl()+"/loginUser/checkPhoneCode.do", //校验手机验证码
			           async: false ,
...
function userRegist(){
	var phoneno=$("#phoneno").val();
	window.location.href= getUrl()+"/views/quicksure/jsp/registUser.jsp?phoneNo="+phoneno;
}
